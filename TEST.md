...
| [`test_provenance_enforcement.py`](agent_runner/tests/test_provenance_enforcement.py) | 11 | Chart-provenance validation + structural-mimic review (`agent_runner.provenance_enforcement` and `chart_structural_review.py`). Uses **real** powerslides reference SVGs so on-disk checks and topology signatures exercise actual files. Skips if the catalog is absent. |
