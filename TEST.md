...
| [`test_status_pubsub.py`](agent_runner/tests/test_status_pubsub.py) | 10 | GCP Pub/Sub sink of `status_logger.StatusProgressLogger` (fake `pubsub_v1` in `sys.modules`, fully offline): publisher created with **message ordering** enabled, every event published under the per-run ordering key (`RUN_ID`), citation payload shape, a failed publish triggers `resume_publish` (ordering self-heal), `close()` flushes all in-flight futures, and Cloud Run **Jobs** detection (`CLOUD_RUN_JOB`/`CLOUD_RUN_EXECUTION`, not just `K_SERVICE`) incl. default-topic resolution. |
...
_Total: 130 tests._ Keep the counts and the total in sync when you add or remove tests.
