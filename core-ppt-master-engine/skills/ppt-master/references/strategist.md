...
**Non-blocking by default**: After the read, present professional recommendations for the eight items below as a summary, make all design and content decisions autonomously on behalf of the user, and immediately proceed to generate the Design Specification, Content Outline, and SVGs without halting, running all steps continuously. **Exception**: If the user explicitly requests interactive execution, interactive mode, or manual confirmation/halting in their initial input, this checkpoint becomes ⛔ BLOCKING; the AI must wait for explicit user confirmation before proceeding.

> **Execution discipline**: In interactive mode, this is the last BLOCKING checkpoint in the pipeline. In the default continuous mode (or after confirmation in interactive mode), complete the Design Spec and proceed to image generation / SVG / post-processing without further pauses.
