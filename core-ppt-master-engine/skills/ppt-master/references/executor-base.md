...
- Entry present (e.g., `P09: timeline_horizontal`) → adapt the corresponding chart SVG already in context. Apply project colors/typography/density; do not copy verbatim. Cross-reference `templates/charts/charts_index.json` for the chart's purpose summary if needed.
  - **Raw-export exception (company templates only)** → if the loaded chart SVG is a **raw PowerPoint export** — it contains `<filter>` / `feGaussianBlur` / `feComponentTransfer`, or is very large (a single flattened, hundreds-of-elements drawing) — do **NOT** try to redraw or adapt it. Redrawing these from scratch loses the distinctive infographic. Instead **copy the template verbatim into the page and edit only the `<text>` content** for this slide's data (exactly like a mirror page, §1.1). **Leave its colors and font-family attributes as-is** — the runner re-themes raw-template pages to the project palette + typography deterministically after your turn. Geometry, fills-as-drawn, and fonts must be preserved verbatim so that re-theme has the original design to work from.
    - **This verbatim copy is REQUIRED, and is explicitly allowed by [SKILL.md](../SKILL.md) rule 9** — copying one approved page and editing its text is hand-authoring, NOT "script-generated" SVG. Do **NOT** rebuild the infographic from scratch with `write_to_file` "for better control" — a from-scratch rebuild is the exact failure this rule prevents (provenance will say `company`/`<key>` while the slide shows a generic redraw).
    - **Edit DATA text only; preserve the template's identity labels.** Replace campaign names, dates, KPI values, and body copy with the slide's content — but keep any label that names *what the infographic is* accurate to the visual. E.g., a monthly-calendar template's header "MONTHLY MARKETING CALENDAR" must stay **"Monthly…"**; do NOT relabel it "Annual…" to echo the deck title. The title on the slide must describe what the slide actually shows, not the deck.
    - **Respect the cell's text budget — fit, don't overflow.** A raw template's geometry is **fixed**: each `<text>` sits in a cell sized for the *original* label's length, and the layout cannot reflow. Keep every replacement string within roughly the **original string's character count** (a word or two longer is fine; a full phrase where the template had one short word is not). If the slide's content is longer, **abbreviate or split it across the `<text>` lines the template already provides** — do NOT lengthen a single line past its cell. Overstuffing a fixed cell makes text spill across cell borders, and the runner's auto-fit can only shrink so far before it gives up and leaves it crowded. Match the template's text density; that is what keeps the verbatim copy looking like the original.
...
```bash
ls core-ppt-master-engine/skills/ppt-master/templates/icons/chunk-filled/ | grep home
ls core-ppt-master-engine/skills/ppt-master/templates/icons/tabler-filled/ | grep home
ls core-ppt-master-engine/skills/ppt-master/templates/icons/tabler-outline/ | grep chart
ls core-ppt-master-engine/skills/ppt-master/templates/icons/phosphor-duotone/ | grep house
ls core-ppt-master-engine/skills/ppt-master/templates/icons/simple-icons/ | grep github
```
...
> **Exception — raw company-template exports**: when the loaded company chart SVG is a raw PowerPoint export (`<filter>` / `feGaussianBlur` / `feComponentTransfer`, or a very large single flattened drawing), the rule **inverts**: copy it **verbatim** and edit only `<text>` content; do **not** recolor or change fonts. These are too complex to redraw faithfully, so the runner re-themes them to the project palette + typography deterministically after the turn (see [`page_charts` raw-export exception](#per-page-chart-reference--page_charts-section) above). Preserve geometry, colors-as-drawn, and font families verbatim.
...
> Japanese / Korean / Other languages: Follow the same structure, writing naturally in the corresponding language.

**Number readability**: TTS reads digits and symbols literally. Prefer fully-spelled forms in the language being spoken when literal pronunciation would be awkward (e.g. Chinese "sixty-eight percent" reads better than "68%"; "one to two minutes" reads as "one dash two minutes"). Plain integers and percentages in English are fine as-is.
...
- Adding `Key points: (1) …` / `Duration: 2 minutes` / `Flex: …` lines — TTS will speak "Key points One …".
