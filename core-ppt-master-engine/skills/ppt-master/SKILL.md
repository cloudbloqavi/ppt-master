...
> 2. **HARD STOP (interactive mode only)** — Steps marked ⛔ BLOCKING halt only when the user's initial prompt explicitly requested interactive execution or manual confirmation. In the default non-interactive / programmatic mode, proceed autonomously without halting, making the best decision on the user's behalf
> 3. **NO CROSS-PHASE BUNDLING** — Cross-phase bundling is FORBIDDEN. (Note: the Eight Confirmations in Step 4 are **non-blocking by default** — the AI presents recommendations as a summary, makes all design and planning decisions autonomously on behalf of the user, and immediately proceeds to generate the Design Specification, Content Outline, and SVGs without halting, running all steps continuously. **Exception**: If the user's initial prompt explicitly requests interactive execution, interactive mode, or manual confirmation/halting, this step becomes ⛔ BLOCKING and the AI must halt and wait for confirmation).
...
>    - **NOT covered by this rule — verbatim template copy is REQUIRED, not forbidden.** Copying an approved company chart template (a raw PowerPoint-export infographic, see executor-base.md §"Per-page chart reference" raw-export exception) into the page and editing only its `<text>` is **hand-authoring a single page**, not script generation — there is no generator, no batch loop, no templating-from-data. For these raw company templates the Executor MUST copy the file verbatim and MUST NOT redraw it from scratch (rebuilding loses the distinctive infographic). "Hand-written" means *the agent produces the page itself* — copying-then-text-editing one approved page qualifies; it does not mean "redraw every shape from memory."
...
**Non-blocking by default**: present the Eight Confirmations as a summary, make all design and content decisions autonomously on behalf of the user, and immediately proceed to generate the Design Specification, Content Outline, and SVGs without halting, running all steps continuously. **Exception**: If the user explicitly requests interactive execution, interactive mode, or manual confirmation/halting in their initial input, this checkpoint becomes ⛔ BLOCKING; the AI must present the Eight Confirmations and wait for explicit user confirmation or modification before outputting the Design Specification & Content Outline.
...
**Live Preview (Disabled by default)**: before the first SVG, do NOT start the browser editor server and skip starting the live preview by default. **Exception**: If the user explicitly requests to start live preview, turn on live preview, or enable it (e.g., `--preview` or "enable live preview"), start the browser editor in live mode and keep it running continuously through Executor + Step 7 export:
```bash
...
- [x] Live preview started (only if explicitly enabled)
